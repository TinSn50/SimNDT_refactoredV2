...
class EngineController():
    ...
    def __init__(self, simPack, SimNDT_SnapShots, GL= False, IsReceiverPlot = False, statusBarWidget=None, parent= None):

        self.simPack = simPack
        self.SimNDT_SnapShots= SimNDT_SnapShots
        self.GL	= GL
    ...
    def run(self):
        ...
        if Name == "Transmission" or Name == "PulseEcho":
            self._run_SingleLaunch()
            return None
        elif Name == "LinearScan":
            self._run_LinearScan()
            return None
        elif Name == "Tomography":
            self._run_Tomography()
            return None
        else:
            raise ValueError('EngineController.run: Unsupported Inpsection method '+Name)
    ...
    def _chooseModel(self):
        ...
        try:
           ...
           ValueError("EngineController._chooseModel Error "+str(e))
           sys.exit()
        ...
        if self.SimNDT_SnapShots.IsEnable:
            if (self.SimNDT_FD.n % self.SimNDT_SnapShots.Step == 0):
                ...
                if self.SimNDT_SnapShots.enableFields:
                    ...
                    self.SimNDT_FD.clEnqueue(self.SimNDT_FD.Vx, self.SimNDT_FD.Vx_buf)
                    self.SimNDT_FD.clEnqueue(self.SimNDT_FD.Vy, self.SimNDT_FD.Vy_buf)
                    self.SimNDT_SnapShots.save_fields(self.SimNDT_FD.Vx, self.SimNDT_FD.Vy, self.SimNDT_FD.n)
        else:
    ...
    def _run(self, idx):
        ...
        while (self.SimNDT_FD.n < self.TimeSteps):
            ...
            self.saveOptions(step, idx, self.n)
