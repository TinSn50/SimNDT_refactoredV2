...
from SimNDT.core.materialLibrary import getMaterialLibrary
...
def read_scenario(sim_params, SimNDT_geom_objects, SimNDT_Bc_objs):
    ...
    if "Scenario" in sim_params.keys():
        ...
        SimNDT_Scenario.createBoundaries(SimNDT_Bc_objs) ## TODO after addObjects?

        SimNDT_Scenario.updateScenario()           
    else:
      raise ValueError("read_scenario: No Scenario section found, but required.")
...
def read_materials(sim_params,Library):
    ...
    if "Materials" in sim_params.keys():
        ...
        for mat_num in range(len(sim_params["Materials"])):
            ...
            if "Rho" in sim_params["Materials"][mat_num]:
              mat_rho = sim_params["Materials"][mat_num]["Rho"]
              mat_vl = sim_params["Materials"][mat_num]["VL"]
              mat_vt = sim_params["Materials"][mat_num]["VT"]
            else:
              # get it from the library
              mat = Library[mat_name]
              # [rho, lam, mu, VL, VT]
              mat_rho = mat[0]
              mat_lambda = mat[1]
              mat_mu = mat[2]
            ...
            if "Eta_v" in sim_params["Materials"][mat_num]:
            ...
            mat_lambda = mat_rho * (mat_vl**2- 2*(mat_vt**2))
            mat_mu = mat_rho * (mat_vt**2)
            mat_c11 = mat_lambda + 2 * mat_mu
            mat_c12 = mat_lambda
            mat_c22 = mat_c11
            mat_c44 = mat_mu
...
def read_boundaries(sim_params):
    ...
    if "Boundaries" in sim_params.keys():    
        print("Reading {} Boundaries.".format(len(sim_params["Boundaries"])))
        SimNDT_Bc_objs = []
        for bc_num in range(len(sim_params["Boundaries"])):
            ...
            if bc_type == "Absorbing":
              ...
              raise ValueError("Boundary type "+bc_type+" is invalid [Expecting Absorbing or Air].")
            bc_obj = Boundary(bc_name, bc_type_num, bc_size)
...
def read_source(sim_params):
    ...
    if "Source" in sim_params.keys():
        ...
        #! check if Source attributes should be changed by passing as arguments
        SimNDT_Source = Source()
        print("Finished reading Source section.")
...
def read_signal(sim_params):
    ...
    if "Signal" in sim_params.keys():
        ...
        print("Finished reading Signal section.")
...
def read_receivers(sim_params):
    ...
    if "Receivers" in sim_params.keys():
        ...
        print("Finished reading Receivers section.")
...
def SingleLaunchSetup(SimNDT_Scenario, SimNDT_Source, SimNDT_Inspection, SimNDT_Transd_objs, SimNDT_Signal):
    ...
    if SimNDT_Signal.Name == "RaisedCosine":
        if SimNDT_Signal.Frequency > 1e8:
            raise ValueError("SingleLaunchSetup: Frequency out of range (MHz)")
        cycles = 1
    elif SimNDT_Signal.Name == "GaussianSine":
        if SimNDT_Signal.Frequency > 1e8:
            raise ValueError("SingleLaunchSetup: Frequency out of range (MHz)")          
    else:
        ValueError("Invalid Signal Function Name "+SimNDT_Signal.Name)
...
def SimulationSetup(Scenario, SimNDT_Mat_objs, SimNDT_Transd_objs, Simulation):
    ...
    # calculating dt, dx in simulation
...
def openSim(sim_params):
    ...
    - SimNDT_SnapShots: The snapshots object.
    """
    SimNDT_Library = getMaterialLibrary()
